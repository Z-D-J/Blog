...
* 初始化的语句可以拆分为两个步骤：`int[] array; array = new int[5]`或者`int[] array; array = new int[] {1, 2, 3};`
...
* lambda表达式的形式：(parameters) -> expression或(parameters) ->{ statements; }。如：
...
* 当lambda表达式只调用一个方法二不做其他操作时，可以把lambda表达式重写为**方法引用**。
* 方法引用有三种情况：
...
* 同一个网络中的计算机在进行连接和通信时需要遵守一定的规则，这些规则被称为网络通信协议，它对数据的传输格式，速率，步骤等做了统一规定。
* TCP/IP协议：传输控制协议/因特网互联协议是Internet最基本和广泛的协议，它定义了计算机如何炼乳因特网以及数据在它们之间传输的标准。它包含了一系列协议，并且采用了**4层分层模型**，每一层都呼叫它的下一层所提供的协议来完成自己的需求。
